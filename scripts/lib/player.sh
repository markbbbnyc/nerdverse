...
heal_player() {
    local cur_hp max_hp heal_amount new_hp
    cur_hp=$(db_query "SELECT current_hp FROM characters WHERE name='Meyiu';")
    max_hp=$(db_query "SELECT max_hp FROM characters WHERE name='Meyiu';")
...
show_status() {
    ...
    printf '  %sConnection with Sera%s  (earned through shared action — not menu grinding)\n' "$GREEN" "$RESET"
