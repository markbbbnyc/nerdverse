...
# Whimsical player-card visualization — Meyiu | Sera | Enemy
function draw_ascii_combatants() {
    local player_hp=$1 player_max=$2 enemy_hp=$3 enemy_max=$4 enemy_name="$5"
    local sera_hp sera_max
    sera_hp=$(db_query "SELECT current_hp FROM characters WHERE name='Sera Thornwake';")
    sera_max=$(db_query "SELECT max_hp FROM characters WHERE name='Sera Thornwake';")
...
function start_encounter() {
    ...
    while [[ $player_hp -gt 0 && $enemy_hp -gt 0 ]]; do
        ...
        while IFS=$'\t' read -r akey aname adesc uses prof; do
            ...
            printf "  %s) %s\n" "$i" "$display_name"
            ((i++))
        done < <(prog_get_combat_abilities "Meyiu" 2>/dev/null || get_player_abilities)
        ...
        if [[ $defended -eq 0 ]]; then
            ...
            local prof_bonus=0
            local prof
            prof=$(db_query "SELECT proficiency FROM character_abilities WHERE character_id=(SELECT id FROM characters WHERE name='Meyiu') AND ability_key='${ability_key}';")
            prof_bonus=$(( ${prof:-0} / 3 ))
            ...
            if [[ $is_spell -eq 1 ]]; then
                db_exec "UPDATE character_abilities SET proficiency = COALESCE(proficiency,0) + 1
                         WHERE character_id = (SELECT id FROM characters WHERE name='Meyiu')
                         AND ability_key LIKE '%firebolt%';"
                prog_practice "Meyiu" "arcane,focus" 1
            elif [[ $defended -gt 0 ]]; then
                prog_practice "Meyiu" "practical,strength" 1
            fi
        else
            prog_practice "Meyiu" "practical,strength" 1
        fi
